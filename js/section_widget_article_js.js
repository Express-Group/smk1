...
$().ready(function() {		
	...
	/*	if(main_section_text == 'Nation') {
		...
		//alert($('#customImage_' + WidgetCustomImage.content_id + '_view').attr('my_attr') );
				...
				$.ajax({
					url: base_url+"smcpan/section_widget_articles/custom_image_upload",
					type: "POST",					
					data: formData,
					contentType:false,
					cache: false,
		...
		$(document.body).on('click',"#browse_image_insert",function() {			
			...
			var contentType = '1';
				...
				if($("#browse_image_id").val() != '' && $("#browse_image_id").val() != 0 ) {
					...
					if($("#browse_image_id").data('image_source')) {
						...
						//var image_id = ImageDetails.content_id;
						...
						//insertImageIntocustomTemp(ImageData, true, image_id);
				...
				var ImageIndex = '';
				/*if(image_type == 'home') {
					ImageIndex = $("#home_image_gallery_id_" + WidgetCustomImage.content_id).val();
				} */
				
				ImageIndex = $("#Image_content_id" + WidgetCustomImage.content_id).val();
				
				//ImageIndex = $('#uploaded_image_'+ WidgetCustomImage.content_id).val();
				...
				if(ImageIndex != '') {
					...
					ImageData = "alt="+image_alt+"&caption="+image_caption+"&date=''&height=''&width=''&size=''&path="+image_path+"&content_id="+content_id+"&contentType=1&article_id= " + WidgetCustomImage.content_id + "&instance_id= " + WidgetCustomImage.widget_instance_id + "&mainSectionConfig_id= " + WidgetCustomImage.mainSection_config_id;
				 		
					insertImageIntocustomTemp(ImageData, false, '');					
					window.open(base_url+"smcpan/section_widget_articles/custom_image_processing/"+encodeURIComponent(base64_encode(ImageIndex)), '_blank');*/
					
					$("#home_physical_name"+ WidgetCustomImage.content_id).attr('readonly',false);
					
					window.open(base_url+"smcpan/section_widget_articles/custom_image_processing/"+encodeURIComponent(base64_encode(ImageIndex))+"/"+WidgetCustomImage.support_image_width+"/"+WidgetCustomImage.support_image_height, '_blank');		
			...
			$(document.body).on('click', '#delete_article_image' ,function(){
				...
				if(ImageIndex != '') {
					...
					$.ajax({
						url: base_url+"smcpan/article_image/delete_temp_image",
						type: "POST",
						data: ImageData,
						dataType: "json",
						async: false, 
			...
			$('#example').dataTable( {
				...
				"ajax": {
					"url": base_url+"smcpan/article/search_internal_article",
					"type" : "POST",
					"data" : {
					...
					if(reached_last){					
						...
						if($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight){
							$container.append('<div class="LoadingSelectImageLocal"><img src="'+ base_url +'images/admin/loadingimage.gif"><br>Loading ...</div>');							
								$.ajax({
									url: base_url+"smcpan/section_widget_articles/search_image_library_scroll/"+ scroll_down_inc, // Url to which the request is send
									type: "POST",             // Type of request to be send, called as method
									dataType: "HTML",
									complete: 	function(){													
...
function insertImageIntocustomTemp_old(ImageData, is_from_save, image_content_id)
{
	...
	$.ajax({
			...
			success	: function(data) {
							...
							if(is_from_save)
							{
								...
								$('#image_library_id'+ WidgetCustomImage.content_id).val(image_content_id);
