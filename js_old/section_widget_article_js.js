...
$().ready(function() {		
	...
	/*	if(main_section_text == 'Nation') {
		...
		//alert($('#customImage_' + WidgetCustomImage.content_id + '_view').attr('my_attr') );
				...
				$.ajax({
					url: base_url+"niecpan/section_widget_articles/custom_image_upload",
					type: "POST",					
					data: formData,
					contentType:false,
					cache: false,
		...
		$(document.body).on('click',"#browse_image_insert",function() {			
			...
			var contentType = '1';
				...
				if(ImageIndex != '') {
					...
					window.open(base_url+"niecpan/section_widget_articles/custom_image_processing/"+encodeURIComponent(base64_encode(ImageIndex)), '_blank');*/
					
					$("#home_physical_name"+ WidgetCustomImage.content_id).attr('readonly',false);
					
					window.open(base_url+"niecpan/section_widget_articles/custom_image_processing/"+encodeURIComponent(base64_encode(ImageIndex))+"/"+WidgetCustomImage.support_image_width+"/"+WidgetCustomImage.support_image_height, '_blank');		
			...
			$(document.body).on('click', '#delete_article_image' ,function(){
				...
				if(ImageIndex != '') {
					...
					$.ajax({
						url: base_url+"niecpan/article_image/delete_temp_image",
						type: "POST",
						data: ImageData,
						dataType: "json",
						async: false, 
			...
			$('#example').dataTable( {
				...
				"ajax": {
					"url": base_url+"niecpan/article/search_internal_article",
					"type" : "POST",
					"data" : {
					...
					if(reached_last){					
						...
						if($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight){
							$container.append('<div class="LoadingSelectImageLocal"><img src="'+ base_url +'images/admin/loadingimage.gif"><br>Loading ...</div>');							
								$.ajax({
									url: base_url+"niecpan/section_widget_articles/search_image_library_scroll/"+ scroll_down_inc, // Url to which the request is send
									type: "POST",             // Type of request to be send, called as method
									dataType: "HTML",
									complete: 	function(){													
