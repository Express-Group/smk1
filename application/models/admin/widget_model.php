...
class Widget_model extends CI_Model
{
 ...
 public function get_content_type($content_id)
	...
	public function GetPrintEdition($mode){
		$class_object = new Widget_config_class;
		return $class_object->GetPrintEdition($mode);
	}
...
class Widget_config_class extends Widget_model
{
	public function GetPrintEdition($mode){
		if($mode=='adminview'){
			$this->db->select(array('print_url','print_image'));
			$data=$this->db->get('settings');
		}else{
			$this->live_db->select(array('print_url','print_image'));
			$data=$this->live_db->get('settings');
		}
		return $data->result();
