...
<div class="Container">
	<div class="BodyWhiteBG">
		...
		{ ?>
             <p class="FloatRight SaveBackTop remoda1-bg"><a href="<?php echo base_url();?>smcpan/rolemaster_manager/role_form_view" type="button" class="btn-primary btn"><i class="fa fa-plus-circle"></i>Add New</a></p>
             <?php }?>
	...
	function delete_rolemaster(id)
		{
			...
			if(r==true){
			window.location.href="<?php echo base_url() ?>smcpan/rolemaster_manager/rolemaster_delete/"+id;
		}
	}
</script>
<script>
	...
	$('#example').dataTable( 
{
    ...
    $('#example').dataTable( {
      ...
      if ($(this).find('tbody tr').length<=1) {
		...
		"fnDrawCallback":function(oSettings )
		{
		   ...
		   if($(this).find('tbody tr').text()== "No matching records found")
		   {
			  ...
			  $("#example_length").hide();
			  // $("#example").find('tbody tr').append($('<td class="BackArrow"><a href="<?php  echo base_url(); ?>smcpan/rolemaster_manager" data-toggle="tooltip" title="Back to list"><i class="fa fa-reply fa-2x"></i></a></td>'));
	 ...
	 $("#example").find('tbody tr').html($('<td valign="top" colspan="10" class="dataTables_empty BackArrow">No matching records found <a href="" data-toggle="tooltip" title="Back to list"><i class="fa fa-reply fa-2x"></i></a></td>'));
		...
		"ajax": {
            "url": "<?php echo base_url(); ?>smcpan/rolemaster_manager/roledepartment_datatable",
			"type" : "POST",
			"data" : {
