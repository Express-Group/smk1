...
{
	var dataTableAjaxUrl = base_url + "smcpan/section_widget_articles/get_widgetInstance_related_article";
}
else
{
	var dataTableAjaxUrl = base_url + "smcpan/section_widget_articles/get_widgetInstancearticle";
}
...
var subSectionConfig_Id 	= $('#subSectionConfig_Id').val();	
	...
	var image_id = $('#imageID_'+d.ID).val();
		...
		var img_caption = '<input type="text" id="home_image_caption' + d.ID + '" name="home_image_caption' + d.ID + '" value="'+ custom_image_caption + '" class="margin-top-5 WidthFull"  >';
		var img_alt		= '<input type="text" id="home_image_alt' + d.ID + '" name="home_image_alt' + d.ID + '" value="'+ custom_image_alt + '" class="margin-top-5 WidthFull"   >';
    ...
    $('#example tbody').on('click', '.widget_toggle', function () {
		...
		if ( row.child.isShown() ) 
		{
			...
			if(content_id != '' && content_id != 0){	
				...
				if(content_id != '' && content_id != 0 && typeof content_id != 'undefined'){	
		...
		$(".save_custom_details").on("click", function (e) {
			...
			//if(!remove_image)
			{
					...
					if( $("#customImage_" + content_id + "_view").attr('src') != '' )
					{
						...
						$.ajax({
							url: base_url+"smcpan/section_widget_articles/check_custom_image_name", // Url to which the request is send
		...
		function check_image_name_exists()
			...
			postdata = "physical_name="+physical_name+'.'+$("#physical_name"+get_content_id).attr('physical_extension')+"&temp_id="+$("#physical_name"+get_content_id).attr('rel');
				$.ajax({
					url: base_url+"smcpan/common/check_image_name", // Url to which the request is send
					type: "POST",             // Type of request to be send, called as method
					data:  postdata,
					dataType: "json",
					async: false, 
...
/*function get_subsections(mainsectionConfigId)
{
	...
	$.ajax({			
		url: base_url + "smcpan/section_widget_articles/get_instanceSubsectionConfig",
		type: 'post',
		async: false,
...
function savePermanent(istemporary)
{
	...
	{ 		
		...
		{
			save_url = base_url + "smcpan/section_widget_articles/add_widget_article/saveTemporary";
		}
		else if(istemporary == 'saveTemporary-savepermanent-publish_articles')
		{
			save_url = base_url + "smcpan/section_widget_articles/add_widget_article/publish/s";
		...
		{
			save_url = base_url + "smcpan/section_widget_articles/add_widget_article/saveTemporary-savepermanent/s";
	...
	<div class="BodyWhiteBG">
		...
		<div class="Overflow DropDownWrapper">
			<div class="previewcontainer wid-main">
				<form name="article_popup_form" id="article_popup_form" action="<?php echo base_url()."smcpan/section_widget_articles/add_widget_article/saveTemporary"; ?>" method="post" enctype="multipart/form-data">
...
//				 }
	...
	//console.log(required_values);
		$.ajax({
			url			: base_url+"smcpan/section_widget_articles/view_temparticles",
			method		: 'post',
			async		: async_status,
...
{		
	$.ajax({
			url		: base_url+"smcpan/section_widget_articles/Insert_temp_from_image_library",
			type	: "POST",
			data	: ImageData,
			dataType: "json",
			async	: false, 	
	...
	<div class="article_popup GalleryPopup ArticlePopup" style="height: 467px;">
		...
		<div class="article_popup2">
			<div class="article_upload">
				<form  name="ImageForm" id="ImageForm" action="<?php echo base_url(); ?>smcpan/section_widget_articles/custom_image_upload" method="POST" enctype="multipart/form-data">
			...
			<div class="article_browse1">
				...
				<?php if(isset($image_library)) { 
						 ...
						 else
						 {
						  ...
						  ?>
				<nav id="page-nav"> <a href="<?php echo base_url(); ?>smcpan/section_widget_articles/search_image_library_scroll/2"></a> </nav>
			</div>
	...
	jqis(".set_image").click(function(){
	 ...
	 var $container = jqis('.popup_images');
		...
		$container.html('<div id="LoadingSelectImageLocal" style="display: block;"><img src="<?php echo base_url(); ?>images/admin/loadingimage.gif" style="border:none; width:23px; height:23px;text-align: center;float: none;margin: 0px;padding: 0px;"><br>Loading ...</div>');
				...
				$.ajax({
				url: base_url+"smcpan/section_widget_articles/get_image_library_scroll/1", // Url to which the request is send
				type: "POST",             // Type of request to be send, called as method
				dataType: "HTML",
...
$container.html('<div id="LoadingSelectImageLocal" style="display: block;"><img src="<?php echo base_url(); ?>images/admin/loadingimage.gif" style="border:none; width:23px; height:23px;text-align: center;float: none;margin: 0px;padding: 0px;"><br>Loading ...</div>');
	...
	jqis.ajax({
		url: base_url+"smcpan/section_widget_articles/search_image_library",
		type: "POST",
		data: postdata,
		dataType: "json",
		success: function(data){
...
{	
   ...
   if(confirm(confirm_text))
	  {	
		...
		$.ajax({
				url: base_url + "smcpan/section_widget_articles/widgetarticle_customdetails",
				type: 'post',
				processData: false,
				contentType: false,
				async: false,
