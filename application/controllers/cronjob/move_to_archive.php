...
{	
	...
	public function index() {
		...
		$end_date = date('Y-m-d',(strtotime ( '-'.$Days.' day' , strtotime( date('Y-m-d')) ) ));
		$start_date = date('Y-m-d',strtotime('01-01-0001 00:00'));
		
		$get_content_type		= $this->common_model->get_content_type();
		
		foreach($get_content_type as $getcontenttype) {
			
		if($getcontenttype->contenttype_id != 2) {
			
		$Status  = $this->move_to_archive_model->content_move_to_archive($getcontenttype->contenttype_id,$start_date, $end_date);	
					...
					$fp = fopen($LogPath,"wb");
						...
						if(file_exists($LogPath))  {
							...
							foreach($Status as $key => $Value) {
								...
								$txt = 	"Content Type 	: ".$getcontenttype->contenttype_id."\n";
								$txt .=  "Date			: ".date('Y-m-d H:i:s')."\n";
								$txt .= "Content Id 	: ".$key."\n";
								$txt .= "Cronjob Status : ".$Value['status']."\n";
			...
			if(count($Status) == 1 && isset($Status[0])  &  isset($Status[0]['status']) == "Failure" ) {
				echo "Archive is facing issues...please try again";
				exit;
			} else {
				echo "Contents have been archived";
				exit;
			}
			*/
		}
